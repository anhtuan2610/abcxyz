...
import { getTeamMembersByLeader } from "../../services/leader-api";
...
export default function LeaderTeamManagement() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["fetchTeamMemebersByLeader"],
    queryFn: async () => {
      const response = await getTeamMembersByLeader();
      return response;
    },
  });
  ...
  return (
    <div className="relative overflow-x-auto sm:rounded-lg">
      <div className="flex flex-col space-y-4 p-5 bg-gradient-to-r from-blue-200 to-cyan-200 rounded-lg shadow-md">
        <div className="flex justify-center items-center space-x-2">
          <h1 className="text-lg font-semibold text-gray-800">Team Members</h1>
          <svg
      ...
      <div className="mx-auto max-w-screen-sm text-center mb-8 lg:mb-16 cursor-default">
        <h2 className="mb-4 text-4xl tracking-tight font-bold text-gray-700 dark:text-gray-300">
          About the team
        </h2>
        <p className="font-light text-gray-900 lg:mb-16 sm:text-xl dark:text-gray-400">
          Explore the whole collection of open-source web components and
          elements built with the utility classes from Tailwind
        </p>
      </div>
      <div className="grid gap-8 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 p-2">
        {data?.data.map((member) => (
          <div
            key={member.id}
            className={`p-4 border rounded-lg shadow-md bg-white hover:shadow-lg transition duration-200 ${
              member.role === "Leader" ? "border-blue-500" : "border-gray-300"
            }`}
          >
            <div className="flex flex-col items-center">
              <div
                className={`h-16 w-16 rounded-full flex items-center justify-center font-bold text-xl mb-4 ${
                  member.role === "Leader"
