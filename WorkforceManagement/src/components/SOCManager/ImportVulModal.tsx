...
import { importVulnerabilities } from "../../services/soc-manager-api";
...
export default function ImportVulModal({ setIsShowImportModal, id }: TProps) {
  ...
  const { mutate } = useMutation({
    mutationFn: async () => {
      ...
      return await importVulnerabilities({
        projectId: Number(id),
    ...
    onSuccess: (response) => {
      if (response) {
        ...
        setIsShowImportModal(false); // Đóng modal sau khi thành công
      }
    },
    onError: (error) => {
  ...
  const handleImportFile = () => {
    if (!selectedFile) {
      toast.error("Please choose at least 1 HTML file");
      return;
    }
    mutate();
  };
