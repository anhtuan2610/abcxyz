    ...
    <div>
          ...
          {data?.data ? (
            data.data.vulnerabilities.map((item) => (
              <tr key={item.id} className="bg-white border-b hover:bg-gray-50">
                ...
                <td className="px-6 py-4">{item?.assignedAt || "N/A"}</td>
      ...
      >
        <span className="text-sm font-normal text-gray-500 mb-4 md:mb-0 block w-full md:inline md:w-auto">
          Showing{" "}
          <span className="font-semibold text-gray-900">{data?.data.vulnerabilities.length}</span> of{" "}
      ...
      {isShowImportModal && (
        <ImportVulModal setIsShowImportModal={setIsShowImportModal} id={id} />
      )}
      {isShowAssignVulModal && selectedVulId && (
        <AssignVulModal
